...
import { getTopicById } from "@/data/courses/algorithms/data";
import { getUnitLessons } from "@/lib/learn/lessons";
import styles from "@/app/(main)/practice/page.module.css";
...
function buildChapterView(topicId: string) {
...
export default function CourseDetailPage({
  ...
  const courseView = buildChapterView(topicId);

  if (!courseView) {
    return null;
  }

  const { activeTopic, chapters } = courseView;
  const totalUnits = activeTopic.units.length;
  const totalExercises = activeTopic.units.reduce(
    (sum, unit) => sum + getUnitLessons(unit).length,
    0,
  );
  const completedUnits = activeTopic.completed.length;
  const overallProgress = Math.round((completedUnits / Math.max(totalUnits, 1)) * 100);
          ...
          <section className={styles.mainColumn}>
            <article className={styles.descriptionCard}>
              ...
              <div className={styles.descriptionTextWrap}>
                <p>
                  {courseDescription} This course mixes concise theory with hands-on exercises so
                  you can practice concepts in realistic tasks, validate your reasoning, and build
                  steady momentum chapter by chapter.
                </p>
              </div>
          ...
          <aside className={styles.sideColumn}>
            ...
            <section className={styles.sideCard}>
              ...
              <ul className={styles.trackList}>
                <li>
                  <Link href="/courses" className={styles.trackLink}>
                    Algorithms Path
                  </Link>
                </li>
                <li>
